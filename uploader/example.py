...
class Uploader:
  ...
  # 建议先实现 handle 方法，然后执行 `python3 test.py 驱动器名` 测试得到最大文件大小
  MAX_BYTES = 10 << 20
  ...
  @classmethod
  def params(cls):
    # 返回图片头的“填充长度”信息，因为在视频播放时，需要把图片头去掉，所以要知道你填充了多少字节在前面（件第24~26行）
  ...
  def handle(cls, file):
    ...
    # 大部分网站都可以靠这个绕过，若不需要可以将下面一行注释。其实很多只检查文件名而已，因此只需要伪造文件名（见第32行）
